(ns aquaculture.actor
  "AquacultureOperationsActor — the ISCO-08 6221 community aquaculture actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one aquaculture operation request
  ...
  The unconditional invariant: the AquacultureOperationsAdvisor can never
  directly commit a record the AquacultureOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [aquaculture.advisor :as advisor]
            [aquaculture.governor :as governor]
            [aquaculture.store :as store]))

(defn build-graph
  "Build a compiled AquacultureOperationsActor graph. `store` implements
  `aquaculture.store/Store`. `advisor` implements
  `aquaculture.advisor/Advisor` (defaults to `mock-advisor`).
